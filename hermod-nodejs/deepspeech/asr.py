...
class Audio(MqttService):
    ...
    def __init__(self, callback=None, device=None, input_rate=RATE_PROCESS, file=None):
        def proxy_callback(in_data, frame_count, time_info, status):
            #self.log(['callback',len(in_data), frame_count, time_info, status])
            #pylint: disable=unused-argument
            if self.chunk is not None:
                in_data = self.wf.readframes(self.chunk)
                #self.log(['indata',in_data])
            callback(in_data)
        ...
        if self.device:
            ...
            self.log('read file {}'.format(file))
            self.chunk = 320
...
if __name__ == '__main__':
    ...
    parser.add_argument('-w', '--savewav',
                        help="Save .wav files of utterences to given directory", default='./recordings')
    parser.add_argument('-f', '--file',
                        help="Read from .wav file instead of microphone")
#, default='/projects/hermod/hermod-nodejs/tests/audio/test.wav'
    parser.add_argument('-m', '--model',
                        help="Path to the model (protocol buffer binary file, or entire directory containing all standard-named files for model)", default='/projects/deepspeech-0.6.1-models/')
