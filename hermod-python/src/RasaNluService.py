...
class RasaNluService(MqttService):
    ...
    def __init__(
        ...
        self.subscribe_to = 'hermod/+/nlu/parse'
        self.interpreter = interpreter = Interpreter.load(config.get('model_path','./rasa/models/nlu'))
    ...
    def on_message(self, client, userdata, msg):
        ...
        if topic == 'hermod/' + site + '/nlu/parse':
            result = interpreter.parse(payload.text)
            self.log(result)
            self.client.publish('hermod/'+site+'/nlu/intent',data)
